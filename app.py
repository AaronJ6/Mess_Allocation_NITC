...
def reset(hostel_id):
    ...
    for i in range(1,6):
        coco=estab_connection()
        count=coco.execute( 'SELECT COUNT(*) FROM Student WHERE Mess_Id = ? AND Hostel_Id = ?', (i, hostel_id)).fetchone()
        allocated=coco.execute( 'SELECT Allocated FROM Mess_Details WHERE Mess_Id = ?', (i,)).fetchone()
        coco.close()
        t_allocated=int(allocated[0])
        t_count=int(count[0])
        t_allocated=t_allocated-t_count
        coco=estab_connection()
        coco.execute('UPDATE Mess_Details SET Allocated = ? WHERE Mess_Id = ?', (t_allocated, i))
        coco.commit()
        coco.close()
    conn = estab_connection()
    conn.execute(
